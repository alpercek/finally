<template><Bounded as="section" :style="{'border-top-color':slice.primary.color}" class="bg-white border-t-2 !pb-0">
  <div v-if="slice.primary.open" class="hidden md:block">
    <div @click="sshowMobileMenu = !sshowMobileMenu" class="flex items-center justify-between h-7"><PrismicRichText :field="slice.primary.namedateyear" class="px-4 md:px-0 font-circular translate-y-1 text-sm"/><b v-if="sshowMobileMenu">&#9660;</b><b v-else>&#9650;</b></div><div :class="{ aactive: !sshowMobileMenu}" class="max-h-0 transition-all overflow-hidden relative">
      
    <VueSlickCarousel ref="acarousel" :variableWidth="true"  :arrows="false" :autoplay="true" >
  ...
  <div :class="{ aaaaactive: ssshowMobileMenu}" class="transition-all max-h-0 md:max-h-none grid grid-cols-1 items-center justify-items-start pt-1 px-4 md:px-0"><PrismicRichText :field="slice.primary.title" class=" hidden md:block font-metrikbold"/><div class="block md:hidden items-baseline flex gap-2"><PrismicRichText :field="slice.primary.title" class="!text-xl pb-4" @click="ssshowMobileMenu = !ssshowMobileMenu"/><b v-if="ssshowMobileMenu">(-)</b><b v-else>(+)</b></div>
      <PrismicRichText :field="slice.primary.description" class="max-w-2xl font-metrik" /></div>
  </div></div>
  <div v-if="!slice.primary.open" class="hidden md:block">
    <div @click="sshowMobileMenu = !sshowMobileMenu" class="flex items-center justify-between h-7"><PrismicRichText :field="slice.primary.namedateyear" class="px-4 md:px-0 font-circular translate-y-1 text-sm"/><b v-if="sshowMobileMenu">&#9650;</b><b v-else>&#9660;</b></div><div :class="{ aactive: sshowMobileMenu}" class="max-h-0 transition-all overflow-hidden relative">
      
    <VueSlickCarousel ref=uid :variableWidth="true" :arrows="false" :autoplay="true" >
      <div v-for="(item, i) in slice.items" :key="`slice-item-${i}`" class="relative">    
          <PrismicImage :field="item.image" class="h-96 pr-4"/>
          <PrismicRichText :field="item.fig" class="px-4 md:px-0 max-w-xs text-gray-300 !pl-1 text-sm"/>
          <div class="absolute inline-0 z-40 h-full w-full top-0 hidden md:block"></div>
  ...
  <div class="md:hidden block">
    <div @click="sshowMobileMenu = !sshowMobileMenu" ><PrismicRichText :field="slice.primary.namedateyear" class="px-4 text-2xl font-circular"/></div><div :class="{ aactive: sshowMobileMenu}" class="max-h-0 transition-all overflow-hidden relative">
